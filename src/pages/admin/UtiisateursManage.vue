...
<template>
  <q-page class="q-pa-lg">
    <q-card>
      <q-card-section>
        <div class="text-h4">{{ $t('client.title') }}</div>
        <div class="text-caption">{{ $t('client.subtitle') }}</div>
      </q-card-section>
      ...
      <q-card-section class="q-pa-md">
                  ...
                  <q-card-section class="q-pt-none q-pb-md">
                    ...
                    <div class="q-pb-md">
                      <q-input
                        ...
                        label="Sms"
                        v-model="projetQSms"
                        placeholder="Quantite de sms projet"
                        dense
                      />
                    </div>
                    <div class="q-pb-md">
                      Choisissez les licences a utiliser :

                      <q-select
                        autofocus
                        label="licences a utiliser "
                        filled
                        stack-label
                        v-model="selectLicence"
                        :rules="[
                          (val) =>
                            val !== 'Aucun' ||
                            'Veuillez selectionner une licence ',
                        ]"
        ...
        <div class="border-radius-inherit">
          ...
          <div class="q-py-lg q-px-sm" id="details">
            <q-inner-loading v-if="loadingTable" :showing="loadingTable">
...
export default defineComponent({
  ...
  setup() {
    ...
    const optionsLicence = ref([{ id: 0, label: 'Aucun' }]);
    const selectLicence = ref({ id: 0, label: 'Aucun' });
    ...
    const getLSA = async () => {
      ...
      if (response.status) {
        ...
        optionsLicence.value = response.data.map((x: any) => {
          return {
        ...
        console.log('***********************************------');
        console.log(optionsLicence.value);
        console.log('aaaaaa***********************************------');
      } else {
        $q.notify({
          message: 'Erreur lors du chargement des vos licences',
          type: 'negative',
        });
      }
      ...
      if (
        ...
        if (type == 0) {
          data = {
            ...
            emetteurId:
              selectH == 0 ? idManageClient.value : userSelect.value.id,
            routeId: pays.value.routes[0].id,
            quantite: nombre.value,
            // modePaiement: paiementSelect.value.id
          };
        } else if (type == 1) {
          data = {
            typeTransfert: 1,
            emetteurId: Cookies.get('id'),
            recepteurId:
              selectH == 0 ? idManageClient.value : userSelect.value.id,
            routeId: pays.value.routes[0].id,
            quantite: nombre.value,
    ...
    const imprimer = async () => {
      ...
      imgheader.src = 'http://localhost:8080/assets/devoo_blue.png';
    ...
    const newProjet = async () => {
      if (parseInt(projetQSms.value) >= 20) {
        ...
        const response: any = await new RequestApi().newProjetAction(data);
        //  console.log('get qte statu', response.data);
        if (response.status) {
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-unsafe-call,@typescript-eslint/no-explicit-any
          ...
          $q.notify({
            message: 'Creation de projet effectuee avec succes',
            type: 'positive',
          });
        } else {
          laodingCreateP.value = false;
          $q.notify({
            message: 'Erreur lors de la creation du nouveau projet',
            type: 'negative',
          });
        }
      } else {
        $q.notify({
          message: 'minimum 20 sms pour la creation du nouveau projet',
          type: 'negative',
        });
      }
    };
